...
- [x] Per-array length model (`len(arr)` → `len_arr` symbolic constant, bounds checking uses `len_<name>`)
- [ ] Full IEEE 754 float arithmetic via Z3 FPA theory (pending z3 crate rounding mode API)
