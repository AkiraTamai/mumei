...
## 6. 数値型と安全限界 (Numeric Safety)
   i64 への最適化: 現バージョンの検証エンジンは i64 (Int64) に最適化されている。

オーバーフローの自己責任: 算術演算においてオーバーフローが懸念される場合は、requires で値の範囲を制限すること。将来のアップデートにより、境界チェックは自動化され、この明示的な制約は不要になる予定である。

精度の代替: 浮動小数点が未実装の間は、固定小数点方式（1000倍して整数として扱う等）を推奨する。f64 サポート後は、丸め誤差を含めた厳密な検証が可能になる。
